...
class CircumstanceIndicatorListTransformSpec extends SpecBase with ScalaCheckPropertyChecks {
  ...
  val validData =
    Json
      ...
      |    "tr": "Posta ve ekspres gönderiler"
      |  }
      |}
      |""".stripMargin)
      .as[JsObject]
  ...
  "transform" - {
    "when the json matches the expected schema" - {
      "returns the transformed data as a JsSuccess" in {
        ...
        val result = Transformation(CircumstanceIndicatorList).runTransform(validData)
        ...
        "when top level mandatory fields are missing" in {
          forAll(mandatoryFields) {
            mandatoryField =>
              ...
              val result =
                Transformation(CircumstanceIndicatorList).transform
                  .reads(dataWithError)
                  .asEither
                  .left
                  .value
